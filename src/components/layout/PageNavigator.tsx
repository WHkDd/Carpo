...
export function PageNavigator() {
  ...
  if (!isPdf || total <= 1) return null;

  const atFirst = page <= 1;
  const atLast = page >= total;

  return (
    <div className="absolute left-2 top-2 z-10 flex h-7 items-center gap-1.5 rounded-lg border border-border/60 bg-background/75 px-2 text-[12px]">
      <button
        type="button"
        onClick={prevPage}
        disabled={atFirst}
