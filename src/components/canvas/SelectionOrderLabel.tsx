...
function SelectionOrderLabelImpl({ x, y, order }: SelectionOrderLabelProps) {
  const PAD = 4;
  const FONT_SIZE = 11;
  const MIN_W = 18;
  const H = 18;
  ...
  const bgFill = useMemo(() => readHslVar("--primary") || "#262626", []);
  const textFill = useMemo(() => readHslVar("--primary-foreground") || "#f7f7f5", []);
