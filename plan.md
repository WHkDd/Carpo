...
- ~~T5.6~~ (2026-05-12): `src/store/jobSlice.ts` owns an `activeJob` machine (`running → cancelling → cancelled` / `→ done` / `→ error`). `ProgressDialog.tsx` renders against the slice — non-closable backdrop while in-flight, close button only on terminal states. The cancel path calls `markCancelling()` then `cancelJob(jobId)`; the worker honours the token, emits `JobDone { cancelled: true }`, and the slice flips to `cancelled`. AppShell installs **one** global `listen()` subscription per event (`JOB_PROGRESS / JOB_DONE / JOB_ERROR`); the slice dispatchers filter by `job_id` so stray events for unrelated jobs are no-ops. Done-without-cancel pins the bar to 100% (`done = total`) so the user sees the run finish even if the last `JOB_PROGRESS` was missed. Tests: 7 new in `jobSlice.test.ts` covering startJob seed, jobId filtering, terminal-state pinning, `markCancelling` guard, and `clearActiveJob` refusal while running. **Deviation**: no shadcn install — dialog is plain Tailwind + lucide. **Caveat (carried from T5.4)**: the worker's `recognize_with_retry` runs real `[0,2,5]s` sleeps that aren't cancellation-aware, so cancel may take up to ~7s to land if the current block is mid-backoff — exceeds the plan's 1s promise.
...
- ~~T5.7~~ (2026-05-12): `src/lib/format-doc.ts` ports `_on_ocr_finished:2532-2549` line-for-line and is verified byte-for-byte against the Python implementation (5 fixtures in `format-doc.test.ts`, cross-checked with a live `python3` harness). `src/lib/ocr-profile.ts` carries the `OcrProfile → { preview, ocr }` DPI map (standard 150/300, fast 120/200) mirroring `newspaper_ocr.py:160-175`. `src/hooks/useGroupedOcrTrigger.ts` is the call site: validates current file + articles + provider key (and `openai_compatible_base_url` for the custom endpoint), looks up each block rect from `pageStates`, builds the `GroupedOcrRequest`, invokes `startGroupedOcr`, then seeds `jobSlice` with a snapshot of `{ fileId, newspaperName, newspaperDate, requestedArticles: [{id, title}] }`. `src/components/results/ResultDrawer.tsx` is a bottom drawer with a readonly textarea (single-file MVP — per-file tabs are deferred to M6 batch), 复制 wires `tauri-plugin-clipboard-manager` `writeText`, 保存 wires `tauri-plugin-dialog` `save` + `tauri-plugin-fs` `writeTextFile`. AppShell owns the done-listener: on a non-cancelled `JOB_DONE` whose `job_id` matches the active job, it zips `requestedArticles` (id→title) with `results`/`errors`, missing rows get `[识别失败：…]` markers so files aren't silently incomplete, then assembles + opens the drawer. **Deviation (button placement)**: the trigger lives in the **StructureRail bottom footer** (the existing "批量 — 识别选中报道" affordance), not the status bar. Plan said "bottom status bar primary action" but the rail already held a disabled stub from earlier UI passes and the status bar is now visually crowded with provider + profile + zoom controls — surfacing the OCR primary in the rail keeps the status bar a settings/state strip. Single-file MVP retains the plan's anti-duplication rule (one action site). **Deviation (drawer)**: no Radix/shadcn install, so the drawer is plain Tailwind anchored bottom-left/right inside the viewport (`fixed inset-x-2 bottom-2`) rather than a slide-up sheet. Tests: 5 new in `format-doc.test.ts`; `pnpm typecheck + build + 60 vitest`, `cargo check + clippy -D warnings` clean.
...
- ~~T5.8~~ (2026-05-12): `src/components/layout/StatusBar.tsx` grew two `DropMenu` components (provider + OCR profile). Provider menu probes Keychain on mount + on every `settings` mutation; providers without a key are disabled and labelled "未配置" (current provider stays selectable so users can see what's active even without a key). Picking an item dispatches `settingsSlice.setProvider` / `setOcrProfile`, which already persists via `setSettings` IPC. **Deviation**: no Radix install in this project — `DropMenu` is plain Tailwind + lucide with manual outside-click/Esc handling, matching the M3 pattern. The model name is **not** shown in the status bar to keep the footer narrow; it remains visible in the settings dialog. As part of this task `src/components/structure/ProfileToggle.tsx` was deleted (its function moved into the status-bar dropdown) and `BlockOpsPanel` lost its `<ProfileToggle/>` slot.

**Deviation (2026-05-12, UI shuffle)**: settings entry moved out of the toolbar. `Toolbar.tsx` no longer renders a ⚙ button or the empty-queue settings shell — it's the manual-mode toolbar only, and renders `null` when no file is open. The settings entry now lives at the bottom of `QueuePanel` (icon-only in collapsed rail, "⚙ 设置" row in expanded mode), so the configuration affordance is always visible regardless of whether a file is loaded. `AppShell.tsx` passes `onOpenSettings` to `QueuePanel` instead of `Toolbar`. `⌘,` global hotkey unchanged.
...
- ~~T5.9 (static gates)~~ (2026-05-12): all static gates closed — `pnpm typecheck` ✓, `pnpm build` ✓, `pnpm test --run` → 60 passed (8 files), `cargo check` ✓, `cargo clippy -- -D warnings` ✓, 41 cargo tests (paddle 8 + openai 6 + ocr-mod 5 + image 1 + pdf 4 + jobs 15 + plus the wiremock retry/4xx coverage that satisfies the "Inject 429 via mock" line in the bullet above). Document assembly verified byte-for-byte against the Python reference via a live `python3` harness — see `format-doc.test.ts`. **Manual user verification deferred (real API keys, real scans)**: end-to-end with each of the four providers + a full cancel-mid-flight trial belong in the upcoming UI loop (Kai's `pnpm tauri dev` pass). Carry-over caveat from T5.4/T5.6: `recognize_with_retry`'s `[0,2,5]s` sleeps are not cancellation-aware — cancel mid-backoff lands within ~7s rather than the plan's ≤1s. If the manual cancel trial confirms this is visible, follow up by injecting a `tokio::select!` over `token.cancelled()` in the retry loop.
